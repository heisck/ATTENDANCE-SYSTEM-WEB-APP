  ...
  return (
    <div ref={wrapperRef} className="mx-auto w-full max-w-[760px] space-y-3">
      <div className="rounded-xl border border-border/70 bg-background/40 p-3">
        <div className="grid grid-cols-2 gap-2 min-[480px]:grid-cols-3 xl:grid-cols-5">
          <span
            className={cn(
              staticUtilityItemClass,
              "border-primary bg-primary text-primary-foreground shadow-sm"
            )}
            title={`Now: ${sequenceId}`}
          >
            <span className="truncate">Now: {sequenceId}</span>
          </span>
          <span className={staticUtilityItemClass} title={`Next: ${nextSequenceId}`}>
            <span className="truncate">Next: {nextSequenceId}</span>
          </span>
          <span
            className={cn(
              "flex h-9 min-w-0 items-center justify-center rounded-xl px-2 text-center text-[10px] font-medium leading-none sm:h-10 sm:px-3 sm:text-[11px]",
              cueStyles
            )}
            title={`Rotates in ${(Math.max(0, countdownMs) / 1000).toFixed(1)}s`}
          >
            <span className="truncate min-[480px]:hidden">
              {(Math.max(0, countdownMs) / 1000).toFixed(1)}s left
            </span>
